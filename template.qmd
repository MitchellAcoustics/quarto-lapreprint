...
shorttitle: Finite Volume Tutorial
authors:
  - name: Rowan Cockett
    email: "`rowan.cockett`@`ualberta.ca`{=typst}"
...
# Note: Need to use affils, not affiliations for the Typst template to work properly
affils: 
...
venue: "`ar#text(fill: red.darken(20%))[X]iv`{=typst}" # venue: JASA-EL
theme: "`red.darken(50%)`{=typst}"
kind: Notebook Tutorial

# Note: Although the Typst template supports multiple abstracts, this is not yet supported in the Quarto template.
abstract: | 
...
keywords: |
  Finite Volume, Tutorial, Reproducible Research
dates: 
  - title: Published
    year: 2023
    month: 8
...
margins: 
  - title: Key Points
    content: |
      - DC resistivity surveys are used to image subsurface electrical conductivity.
  ...
  - title: Correspondence to
    content: |
      Rowan Cockett rowan@curvenote.com
...
![Derivation of the DC resistivity equations.](files/dc-eqns.png){#fig-dc-eqns width=100%}
...
## In and out {#sec-id-1-in-and-out}

![Geometrical definition of the divergence and the discretization.](files/divergence.png){#fig-div width=100%}
...
As such, we can integrate both sides of the equation over the volume enclosed by the cell. Since $\boldsymbol{D} \boldsymbol{j}$ is constant over the cell, the integral is simply a multiplication by the volume of the cell $\text{v} \boldsymbol{D} \boldsymbol{j}$. The integral of the source is zero unless one of the source electrodes is located inside the cell, in which case it is $q = \pm I$. Now we have a discrete description of equation 1 over a single cell:

$$
v\boldsymbol{D}\boldsymbol{j} = q
$$ {#eq-div}

## Scalar equations only, please {#sec-id-2-scalar-equations-only-please}

Equation @eq-div is a vector equation, so really it is two or three equations involving multiple components of $\vec(j)$. We want to work with a single scalar equation, allow for anisotropic physical properties, and potentially work with non-axis-aligned meshes - how do we do this?! We can use the *weak formulation* where we take the inner product ($\int \vec a \cdot \vec b d v$) of the equation with a generic face function, $\vec f$. This reduces requirements of differentiability on the original equation and also allows us to consider tensor anisotropy or curvilinear meshes.

In @fig-weak-formulation, we visually walk through the discretization of equation (b). On the left hand side, a dot product requires a _single_ cartesian vector, $\boldsymbol{j_x}, \boldsymbol{j_y}$. However, we have a $j$ defined on each face (2 $j_x$ and 2 $j_y$ in 2D!). There are many different ways to evaluate this inner product: we could approximate the integral using trapezoidal, midpoint or higher order approximations. A simple method is to break the integral into four sections (or 8 in 3D) and apply the midpoint rule for each section using the closest $\boldsymbol{j}$ components to compose a cartesian vector. A $\boldsymbol{P}_i$ matrix (size $2 \times 4$) is used to pick out the appropriate faces and compose the corresponding vector (these matrices are shown with colors corresponding to the appropriate face in the figure). On the right hand side, we use a vector identity to integrate by parts. The second term will cancel over the entire mesh (as the normals of adjacent cell faces point in opposite directions) and $\phi$ on mesh boundary faces are zero by the Dirichlet boundary condition. This leaves us with the divergence, which we already know how to do!

![Discretization using the weak formulation and inner products.](files/weak-formulation.png){#fig-weak-formulation width=90%}

The final step is to recognize that, now discretized, we can cancel the general face function $\boldsymbol(f)$ and transpose the result (for convention's sake):

$$
\frac{1}{4} \sum_{i = 1}^4 \boldsymbol{P}_i^T \sqrt{v} \boldsymbol{\Sigma}^{-1} \sqrt{v} \boldsymbol{P}_i \boldsymbol{j} = \boldsymbol{D}^T v \phi
$$ {#eq-weak-formulation}

# All together now {#fig-all-together-now}

We have now discretized the two first order equations over a single cell. What is left is to assemble and solve the DC system over the entire mesh. To implement the divergence on the full mesh, the stencil of $\pm$ 1's must index into $\boldsymbol(j)$ on the entire mesh (instead of four elements). Although this can be done in a `for-loop`, it is conceptually, and often computationally, easier to create this stencil using nested Kronecker Products (see notebook). The volume and area terms in the divergence get expanded to diagonal matrices, and we multiply them together to get the discrete divergence operator. The discretization of the _face_ inner product can be abstracted to a function, $\boldsymbol(M)_f (\sigma^{-1})$, that completes the inner product on the entire mesh at once. The main difference when implementing this is the $\boldsymbol(P)$ matrices, which must index into the entire mesh. With the necessary operators defined for both equations on the entire mesh, we are left with two discrete equations:

$$
\text{diag}(\boldsymbol{v}) \boldsymbol{D} \boldsymbol{j} = \boldsymbol{q}
$$

$$
\boldsymbol{M}_f (\sigma^{-1}) \boldsymbol{D}^T \text{diag}(\boldsymbol{v}) \boldsymbol{\phi} = \boldsymbol{q}
$$

Note that now all variables are defined over the entire mesh. We could solve this coupled system or we could eliminate $\boldsymbol(j)$ and solve for $\phi$ directly (a smaller, second-order system).

$$
\text{diag}(\boldsymbol{v}) \boldsymbol{D} \boldsymbol{M}_f (\sigma^{-1}) \boldsymbol{D}^T \text{diag}(\boldsymbol{v}) \boldsymbol{\phi} = \boldsymbol{q}
$$
...
![Electric potential on (a) tensor and (b) curvilinear meshes.](files/dc-results.png){#fig-results width=90%}
