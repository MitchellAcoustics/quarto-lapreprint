...
#show: doc => template(
  ...
  authors: (
    ( name: "Rowan Cockett",
            affiliations: "1, 2",
                  email: [`rowan.cockett`\@ualberta.ca],
                  orcid: "0000-0002-7859-8394",
             ),
    ( name: "Lindsey Heagy",
            affiliations: "2",
                        orcid: "0000-0002-3710-7181",
             ),
...
", ),
 ...
 (
    title: "Key Points",
    content: [- DC resistivity surveys are used to image subsurface electrical conductivity.
- Finite volume discretization is a common approach to solving partial differential equations.
- The weak formulation allows for anisotropic physical properties and non-axis-aligned meshes.
- The finite volume method is a powerful tool for solving geophysical problems.

],
  ),
 (
    title: "Correspondence to",
    content: [Rowan Cockett rowan\@curvenote.com

],
  ),
 (
    title: "Data Availability",
    content: [Associated notebooks are available on #link("https://github.com/simpeg/tle-finitevolume")[Github] and can be run online with #link("http://mybinder.org/repo/simpeg/tle-finitevolume")[MyBinder];.
...
= DC Resistivity
<sec-dc-resistivity>
DC resistivity surveys obtain information about subsurface electrical conductivity, $s i g m a$. This physical property is often diagnostic in mineral exploration, geotechnical, environmental and hydrogeologic problems, where the target of interest has a significant electrical conductivity contrast from the background. In a DC resistivity survey, steady state currents are set up in the subsurface by injecting current through a positive electrode and completing the circuit with a return electrode \(@fig-dc-setup).
...
The flow of current sets up electric fields according to Ohm’s law, which relates current density to electric fields through the electrical conductivity. From Faraday’s law for steady state fields, we can describe the electric field in terms of a scalar potential, $p h i$, which we sample at potential electrodes to obtain data in the form of potential differences.
...
To set up a solvable system of equations, we need the same number of unknowns as equations, in this case two unknowns \(one scalar, $p h i$, and one vector $arrow(j)$) and two first-order equations \(one scalar, one vector).
...
As such, we can integrate both sides of the equation over the volume enclosed by the cell. Since $bold(D) bold(j)$ is constant over the cell, the integral is simply a multiplication by the volume of the cell $upright("v") bold(D) bold(j)$. The integral of the source is zero unless one of the source electrodes is located inside the cell, in which case it is $q = plus.minus I$. Now we have a discrete description of equation 1 over a single cell:

#math.equation(block: true, numbering: "(1)", [ $ v bold(D) bold(j) = q $ ])<eq-div>
...
#math.equation(block: true, numbering: "(1)", [ $ 1 / 4 sum_(i = 1)^4 bold(P)_i^T sqrt(v) bold(Sigma)^(- 1) sqrt(v) bold(P)_i bold(j) = bold(D)^T v phi.alt $ ])<eq-weak-formulation>

= All together now
<fig-all-together-now>
...
$ upright("diag") (bold(v)) bold(D) bold(j) = bold(q) $

$ bold(M)_f (sigma^(- 1)) bold(D)^T upright("diag") (bold(v)) bold(phi.alt) = bold(q) $
...
$ upright("diag") (bold(v)) bold(D) bold(M)_f (sigma^(- 1)) bold(D)^T upright("diag") (bold(v)) bold(phi.alt) = bold(q) $
